...
class Contrast_Detection_SpF_Gabor_RG:
    def __init__(self, sample_num):
        ...
        self.R = 1
        self.rho_list = np.logspace(np.log10(0.5), np.log10(32), self.sample_num)
        self.contrast_list = np.logspace(np.log10(0.001), np.log10(0.12), self.sample_num)
        self.O = 0
        self.L_b = 100
        self.ppd = 60
...
class Contrast_Detection_SpF_Gabor_YV:
    def __init__(self, sample_num):
        ...
        self.R = 1
        self.rho_list = np.logspace(np.log10(0.5), np.log10(32), self.sample_num)
        self.contrast_list = np.logspace(np.log10(0.001), np.log10(0.8), self.sample_num)
        self.O = 0
        self.L_b = 100
        self.ppd = 60
    ...
    def test_models(self, model_class_instance_list):
        ...
        for model_class in model_class_instance_list:
            ...
            with open(os.path.join(save_root_path,
                      ...
                      'w') as fp:
                json.dump(json_plot_data, fp)
class Contrast_Matching_cos_scale: #这里优化的函数是直接将R和T的cos相似度变得最大
    def __init__(self, sample_num):
        self.W = 224
        self.H = 224
        self.sample_num = sample_num
        ...
        self.L_b = 10
        self.ppd = 60
        self.test_type = 'Contrast Matching Cos Scale'
        self.test_short_name = 'contrast_matching_cos_scale'
    ...
    def generate_pre_figures(self):
        ...
        R_L_array_Con_1 = generate_sinusoidal_grating(W=self.W, H=self.H,
                                                      spatial_frequency=self.rho_referenece,
                                                      orientation=self.O, L_b=self.L_b,
                                                      contrast=1, ppd=self.ppd)
        self.R_L_array_Con_1 = np.stack([R_L_array_Con_1] * 3, axis=-1)
    ...
    def T_optimize_target(self, contrast, rho, model_class, backbone_model, R_feature_Con_0, extreme_T_score_scale, contrast_matching_scale):
        ...
        T_matching_scale = T_score_scale / extreme_T_score_scale
        return (T_matching_scale - contrast_matching_scale) ** 2

    def T_optimize_target_cvvdp(self, contrast, rho, model_class, R_L_array_Con_0, extreme_T_score_scale, contrast_matching_scale):
        T_L_array = generate_sinusoidal_grating(W=self.W, H=self.H,
                                                spatial_frequency=rho,
                                                orientation=self.O, L_b=self.L_b,
                                                contrast=contrast, ppd=self.ppd)
    ...
    def test_models(self, model_class_instance_list):
        ...
        for model_class in model_class_instance_list:
            ...
            for backbone_name in tqdm(backbone_list):
                ...
                backbone_model = model_class.load_pretrained(backbone_name)
                if model_general_name.startswith('cvvdp'):
                    extreme_R_score = float(model_class.compute_score(self.R_L_array_Con_0, self.R_L_array_Con_1).cpu())
                    extreme_R_score_scale = 10 - extreme_R_score # 10 - JOD
                else:
                    R_L_array_Con_0_feature = model_class.forward_feature(backbone_model, self.R_L_array_Con_0)
                    R_L_array_Con_1_feature = model_class.forward_feature(backbone_model, self.R_L_array_Con_1)
                    extreme_R_score = max(min(float(F.cosine_similarity(R_L_array_Con_0_feature.reshape(1, -1), R_L_array_Con_1_feature.reshape(1, -1)).cpu()),1),-1) #cos similarity
                    extreme_R_score_scale = np.arccos(extreme_R_score) / np.arccos(-1)
                for reference_contrast_index in tqdm(range(len(self.reference_contrast_list))):
                    ...
                    contrast_matching_scale = R_score_scale / extreme_R_score_scale
                    ...
                    for rho_test_index in range(len(self.rho_test_list)):
                        ...
                        T_L_array_Con_1 = generate_sinusoidal_grating(W=self.W, H=self.H,
                                                                      spatial_frequency=rho_test_value,
                                                                      orientation=self.O, L_b=self.L_b,
                                                                      contrast=1, ppd=self.ppd)
                        T_L_array_Con_1 = np.stack([T_L_array_Con_1] * 3, axis=-1)
                        if model_general_name.startswith('cvvdp'):
                            extreme_T_score = float(model_class.compute_score(self.R_L_array_Con_0, T_L_array_Con_1).cpu())
                            extreme_T_score_scale = 10 - extreme_T_score  # 10 - JOD
                        else:
                            T_L_array_Con_1_feature = model_class.forward_feature(backbone_model, self.R_L_array_Con_1)
                            extreme_T_score = max(min(float(F.cosine_similarity(R_L_array_Con_0_feature.reshape(1, -1), T_L_array_Con_1_feature.reshape(1, -1)).cpu()),1), -1)  # cos similarity
                            extreme_T_score_scale = np.arccos(extreme_T_score) / np.arccos(-1)
                        bounds = [(0.001, 1)] #Change another way for optimizing
                        if model_general_name.startswith('cvvdp'):
                            target_function = lambda test_contrast: self.T_optimize_target_cvvdp(
                                contrast=test_contrast, rho=rho_test_value, model_class=model_class, R_L_array_Con_0=self.R_L_array_Con_0,
                                extreme_T_score_scale=extreme_T_score_scale, contrast_matching_scale=contrast_matching_scale)
                        else:
                            target_function = lambda test_contrast: self.T_optimize_target(
                                ...
                                backbone_model=backbone_model, extreme_T_score_scale=extreme_T_score_scale,
                                contrast_matching_scale=contrast_matching_scale)
                        optimize_result = differential_evolution(target_function, bounds=bounds, popsize=10)
                        test_contrast_optimization_value = float(optimize_result.x[0])
...
class Contrast_Matching_cos_scale_solve: #这里优化的函数是直接将R和T的cos相似度变得最大
    def __init__(self, sample_num):
        self.W = 224
        self.H = 224
        self.sample_num = sample_num
        ...
        self.L_b = 10
        self.ppd = 60
        self.test_type = 'Contrast Matching Cos Scale Solve'
        self.test_short_name = 'contrast_matching_cos_scale_solve'
        with open(r'gt_json/contrast_constancy_sin_5_cpd.json', 'r') as fp:
            json_data = json.load(fp)
        self.reference_contrast_list = json_data['average_reference_contrast']
        self.generate_pre_figures()
        ...
        for model_class in model_class_instance_list:
            ...
            for backbone_name in tqdm(backbone_list):
                ...
                if model_general_name.startswith('cvvdp'):
                    ...
                    R_L_array_Con_1_feature = model_class.forward_feature(backbone_model, self.R_L_array_Con_1)
                    extreme_R_score = max(min(float(F.cosine_similarity(R_L_array_Con_0_feature.reshape(1, -1), R_L_array_Con_1_feature.reshape(1, -1)).cpu()),1),-1) #cos similarity
                    extreme_R_score_scale = np.arccos(extreme_R_score) / np.arccos(-1) #extreme: 0.274168
                for reference_contrast_index in tqdm(range(len(self.reference_contrast_list))):
                    reference_contrast_value = self.reference_contrast_list[reference_contrast_index]
                    R_L_array = self.R_L_array_matrix[reference_contrast_index]
                    if model_general_name.startswith('cvvdp'):
...
class Contrast_Matching_cos_scale_solve_no_scaler: #这里优化的函数是直接将R和T的cos相似度变得最大
    def __init__(self, sample_num):
        self.W = 224
        self.H = 224
        self.sample_num = sample_num
...
class Contrast_Matching_rafal_cos: #这里优化的函数是直接将R和T的cos相似度变得最大
    def __init__(self, sample_num):
        self.W = 224
        self.H = 224
        self.sample_num = sample_num
        ...
        self.L_b = 10
        self.ppd = 60
        self.test_type = 'Contrast Matching Rafal Cos'
        self.test_short_name = 'contrast_matching_rafal_cos'
        with open(r'gt_json/contrast_constancy_sin_5_cpd.json', 'r') as fp:
            json_data = json.load(fp)
        self.reference_contrast_list = json_data['average_reference_contrast']
        self.generate_reference_figures()

    def generate_reference_figures(self):
        self.R_L_array_matrix = np.zeros([len(self.reference_contrast_list), self.H, self.W, 3])
        for reference_contrast_index in range(len(self.reference_contrast_list)):
            reference_contrast_value = self.reference_contrast_list[reference_contrast_index]
            R_L_array = generate_sinusoidal_grating(W=self.W, H=self.H, spatial_frequency=self.rho_referenece,
    ...
    def T_optimize_target(self, contrast, rho, model_class, backbone_model, R_feature):
        T_L_array = generate_sinusoidal_grating(W=self.W, H=self.H,
                                                spatial_frequency=rho,
                                                orientation=self.O, L_b=self.L_b,
                                                contrast=contrast, ppd=self.ppd)
        T_L_array = np.stack([T_L_array] * 3, axis=-1)
        T_feature = model_class.forward_feature(backbone_model, T_L_array)
        cos_similarity = float(F.cosine_similarity(R_feature.reshape(1, -1), T_feature.reshape(1, -1)).cpu())
        if cos_similarity > 1:
            cos_similarity = 1
        if cos_similarity < -1:
            cos_similarity = -1

        return np.arccos(cos_similarity)

    def T_optimize_target_cvvdp(self, contrast, rho, model_class, R_L_array):
        T_L_array = generate_sinusoidal_grating(W=self.W, H=self.H,
                                                spatial_frequency=rho,
                                                orientation=self.O, L_b=self.L_b,
                                                contrast=contrast, ppd=self.ppd)
        T_L_array = np.stack([T_L_array] * 3, axis=-1)
        JOD = model_class.compute_score(R_L_array, T_L_array)
        return 10 - JOD
            ...
            for backbone_name in tqdm(backbone_list):
                ...
                for reference_contrast_index in tqdm(range(len(self.reference_contrast_list))):
                    ...
                    for rho_test_index in range(len(self.rho_test_list)):
                        rho_test_value = self.rho_test_list[rho_test_index]
                        bounds = [(0.001, 1)]
                        # scale_range = (np.logspace(-3, 0, num=100))
                        # initial_guess = np.array([0.5])
                        if not model_general_name.startswith('cvvdp'):
                            target_function = lambda test_contrast: self.T_optimize_target(
                                contrast=test_contrast, rho=rho_test_value, model_class=model_class,
                                backbone_model=backbone_model, R_feature=R_feature)
                        else:
                            target_function = lambda test_contrast: self.T_optimize_target_cvvdp(
                                contrast=test_contrast, rho=rho_test_value, model_class=model_class,
                                R_L_array=R_L_array)
                        optimize_result = differential_evolution(target_function, bounds=bounds)
                        test_contrast_optimization_value = float(optimize_result.x)
                        match_test_contrast_list.append(test_contrast_optimization_value)
                    json_plot_data[json_backbone_name][
                        f'ref_contrast_{reference_contrast_value}_test_contrast_list'] = match_test_contrast_list
            with open(os.path.join(save_root_path, f'{model_general_name}_test_{self.test_short_name}.json'), 'w') as fp:
