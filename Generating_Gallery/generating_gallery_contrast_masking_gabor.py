...
contrast_test_list = np.logspace(np.log10(0.1), np.log10(0.5), size_of_gallery[0])
default_ppd = 60
...
for contrast_test_index in range(len(contrast_test_list)):
    ...
    for contrast_mask_index in range(len(contrast_mask_list)):
        ...
        T_vid, R_vid = generate_contrast_masking(W=default_W, H=default_H, rho=default_rho, O=default_O,
                                                 ...
                                                 contrast_test=contrast_test_value, ppd=default_ppd, gabor_radius=default_gabor_radius)
...
# plt.show()
plt.savefig('Contrast_Masking_Gabor_sup.png', dpi=300)
plt.close()