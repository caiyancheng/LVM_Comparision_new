...
default_H = 224
default_R = 1
rho_list = np.logspace(np.log10(0.5), np.log10(16), size_of_gallery[1])
contrast_list = np.logspace(np.log10(0.01), np.log10(0.12), size_of_gallery[0])
default_O = 0
default_L_b = 100
default_ppd = 60

fig, axes = plt.subplots(size_of_gallery[0], size_of_gallery[1], figsize=(22, 10.9)) #, dpi=300)
plt.subplots_adjust(left=0.083, right=0.999, top=0.92, bottom=0.001, wspace=0.04, hspace=0.04)
...
for ax, contrast_value in zip(axes[:, 0], contrast_list):
    ...
    # ax.set_visible(True)
sensitivity_list = 1/contrast_list
for ax, sensitivity_value in zip(axes[:, 0], sensitivity_list):
    ax.text(-0.6, 0.5, f'{sensitivity_value:.3f}',
            va='center', ha='center', rotation=90, fontsize=16,
            transform=ax.transAxes)

# 在左侧添加纵向箭头
fig.text(0.01, 0.5, 'Sensitivity = 1 / Contrast', va='center', rotation='vertical', fontsize=20)
fig.text(0.05, 0.5, 'Contrast', va='center', rotation='vertical', fontsize=20)
...
# 添加从上到下的箭头（纵轴）
plt.annotate('', xy=(0.07, 0.003), xytext=(0.07, 0.92),
             arrowprops=dict(facecolor='black', shrink=0.05),
             xycoords='figure fraction')
plt.annotate('', xy=(0.03, 0.92), xytext=(0.03, 0.003),
             arrowprops=dict(facecolor='black', shrink=0.05),
             xycoords='figure fraction')
...
# plt.show()
# plt.savefig('Gabor_SpF_Contrast_sup.png', dpi=300)
# plt.savefig('Real_sup/Gabor_SpF_Contrast_RG_supplementary.svg', dpi=300)
plt.savefig('Sup_pngs/Gabor_SpF_Contrast_RG_supplementary.png', dpi=300)
plt.close()