...
for model_index, model in enumerate(Models):
    ...
    if model == 'No Encoder' or model == 'ColorVideoVDP':
        ...
        model_full_name = rf'$\bf{{{Models[model_index]}}}$' + f'\n {Architecture[model_index]} - {Train_Dataset[model_index]}'
...
fig, axes = plt.subplots(len(plot_dimensions), 1, figsize=(26, len(plot_dimensions) * 2.1))
...
for i, (dimension, adt, ax) in enumerate(zip(plot_dimensions, add_tasks, axes)):
    ...
    for bar, value in zip(bars, score_values):
        ax.text(bar.get_x() + bar.get_width() / 2, max(bar.get_height(), 0) + 0.01, f"{value:.2f}", ha='center',
                va='bottom', fontsize=18, rotation=90)
    ...
    if i == len(plot_dimensions) - 1:
        ...
        ax.set_ylabel("Spearman $r_s$", fontsize=18)
        title_text = adt + ' - ' + dimension + ' - Spearman Correlation ↑'
        ax.set_ylim(0, 1)
    ax.text(0.5, 1.6, title_text, ha='center', va='bottom', fontsize=20, transform=ax.transAxes, fontweight='bold')
    ...
    if i == len(plot_dimensions) - 1:
        ...
        ax.set_xticklabels(plot_model_full_name, rotation=90, fontsize=18, ha='center')
    else:
        ax.set_xticks([])

# 调整图形布局以减少空白区域
plt.subplots_adjust(hspace=1, right=1)
