...
# Create a larger grid: 2 rows and 3 columns (2x2 left, 1x2 right)
fig, axes = plt.subplots(2, 3, figsize=(8, 8), dpi=300)
...
for i, X_dimension in enumerate(X_dimensions_left):
    for j, Y_dimension in enumerate(Y_Alignment_score_dimensions_left):
        ...
        if Y_dimension == 'Phase-Coherent Masking':
            ax.set_ylabel('Phase-Coherent Masking ($r_s$ ↑)')
        if Y_dimension == 'Contrast Matching':
            ax.set_ylabel('Contrast Matching (RMSE ↓)')
        ax.legend()

# Right plots (1x2 grid)
for i, X_dimension in enumerate(X_dimensions_right):
    for j, Y_dimension in enumerate(Y_Alignment_score_dimensions_right):
        ax = axes[i, j + 2]  # Shift the right plots to the last two columns
        X_all_list = []
        Y_all_list = []
...
# Adjust layout
plt.tight_layout()
plt.savefig('combined_plot_1.pdf', format='pdf', dpi=300, bbox_inches='tight', pad_inches=0.05)
# plt.show()
